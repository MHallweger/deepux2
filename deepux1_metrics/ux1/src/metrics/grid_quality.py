...
def execute(b64):
    seg_elements = segmentation.get_elements(b64)['elements']
...
def evaluate(gridValue):
    if gridValue >= 0 and gridValue <= 100:
        evaluation = {'value': gridValue, 'meaning': 'Low', 'evaluation': 'bad'}
    elif gridValue >= 101 and gridValue <= 220:
        evaluation = {'value': gridValue, 'meaning': 'Medium', 'evaluation': 'good'}
    else:
        evaluation = {'value': gridValue, 'meaning': 'High', 'evaluation': 'bad'}

...
def execute(b64):
    ...
    try:
        sym_normalized = (float(sym_key) / float(all_key)) * (
                    (float((all_key - 1) * symmetry_radius) / float(width * height)) ** -1)
...
def evaluate(pixelSymmetry):
    if pixelSymmetry >= 0.00 and pixelSymmetry <= 1.00:
        evaluation = {'value': pixelSymmetry, 'meaning': 'Good', 'evaluation': 'good'}
    else:
        evaluation = {'value': pixelSymmetry, 'meaning': 'Poor', 'evaluation': 'bad'}

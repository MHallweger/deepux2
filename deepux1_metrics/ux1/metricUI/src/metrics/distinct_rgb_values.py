...
class Metric3(AIMMetricInterface):
    ...
    # Public methods
    @staticmethod
    def execute_metric(gui_image: str, gui_type: int = GUI_TYPE_MOBILE) -> int:
        """
        Execute the metric.
        ...
        return n_distinct_rgb_values

    def evaluate(rgbValue: int):
        if rgbValue >= 0 and rgbValue <= 5000:
            evaluation = {'value': rgbValue, 'meaning': 'Less colourful', 'evaluation': 'normal'}
        elif rgbValue >= 15001:
            evaluation = {'value': rgbValue, 'meaning': 'colourful', 'evaluation': 'normal'}
        else:
            evaluation = {'value': rgbValue, 'meaning': 'Fair', 'evaluation': 'good'}

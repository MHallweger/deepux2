...
# First-party modules
from core.constants import GUI_TYPE_MOBILE
from core.interfaces import AIMMetricInterface
...
class Metric6(AIMMetricInterface):
    ...
    # Public methods
    @classmethod
    def execute(cls, gui_image: str, gui_type: int = GUI_TYPE_MOBILE) -> float:
        """
        Execute the metric.
...
def evaluate(contourCongestion):
    if contourCongestion >= 0.00 and contourCongestion <= 0.25:
        evaluation = {'value': contourCongestion, 'meaning': 'Good', 'evaluation': 'good'}
    elif contourCongestion >= 0.26 and contourCongestion <= 0.50:
        evaluation = {'value': contourCongestion, 'meaning': 'Fair', 'evaluation': 'normal'}
    else:
        evaluation = {'value': contourCongestion, 'meaning': 'Poor', 'evaluation': 'bad'}

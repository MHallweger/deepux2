...
class Metric5(AIMMetricInterface):
    ...
    # Public methods
    @staticmethod
    def execute_metric(gui_image: str, gui_type: int = GUI_TYPE_MOBILE) -> float:
        """
        Execute the metric.
        ...
        return figure_ground_contrast

    def evaluate(fgcValue : int):
        if fgcValue > 0.3 and fgcValue < 0.7:
            evaluation = {'value': fgcValue, 'meaning': 'Fair', 'evaluation': 'good'}
        elif fgcValue <= 0.3:
            evaluation = {'value': fgcValue, 'meaning': 'Low Contrast', 'evaluation': 'bad'}
        else:
            evaluation = {'value': fgcValue, 'meaning': 'High Contrast', 'evaluation': 'bad'}

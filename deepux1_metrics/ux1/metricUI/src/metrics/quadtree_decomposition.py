...
def intensity_entropy(inp):
    ...
    p, x = np.histogram(L, bins=l_bins, range=(0, 100), density=True)
    p.ravel()
    p = p * 100.
    p = p + 0.000000000001
...
def evaluate(quadtree_decomposition):
    balance = quadtree_decomposition['balance']
    symmetry = quadtree_decomposition['symmetry']
    equilibrium = quadtree_decomposition['equilibrium']
    numberOfLeaves = quadtree_decomposition['numberOfLeaves']

    if balance >= 0.00 and balance <= 0.65:
        balanceEvaluation = {'value': balance, 'meaning': 'Potential unbalanced', 'evaluation': 'bad'}
    else:
        balanceEvaluation = {'value': balance, 'meaning': 'Balanced', 'evaluation': 'good'}

    if symmetry >= 0.00 and symmetry <= 0.50:
        symmetryEvaluation = {'value': symmetry, 'meaning': 'Poor', 'evaluation': 'bad'}
    else:
        symmetryEvaluation = {'value': symmetry, 'meaning': 'Acceptable', 'evaluation': 'good'}

    if equilibrium >= 0.00 and equilibrium <= 0.65:
        equilibriumEvaluation = {'value': equilibrium, 'meaning': 'Not centralized', 'evaluation': 'bad'}
    else:
        equilibriumEvaluation = {'value': equilibrium, 'meaning': 'Centralized', 'evaluation': 'good'}

    if numberOfLeaves >= 0 and numberOfLeaves <= 1500:
        numberOfLeavesEvaluation = {'value': numberOfLeaves, 'meaning': 'Good', 'evaluation': 'good'}
    elif numberOfLeaves >= 1501 and numberOfLeaves <= 3200:
        numberOfLeavesEvaluation = {'value': numberOfLeaves, 'meaning': 'Fair', 'evaluation': 'normal'}
    else:
        numberOfLeavesEvaluation = {'value': numberOfLeaves, 'meaning': 'Poor', 'evaluation': 'bad'}

    evaluation = {'balance': balanceEvaluation, 'symmetry': symmetryEvaluation,
                  'equilibrium': equilibriumEvaluation, 'numberOfLeaves': numberOfLeavesEvaluation}

...
class GANLoss3(nn.Module):
    ...
    def forward(self, prob, target, reward, _loss2, _loss3):
        ...
        multiloss = torch.sum(p1 * (loss ** 2) + self.cf1, -1) + torch.sum(p2 * (_loss2 ** 2) + self.cf2,
                                                                           -1) + torch.sum(
            p3 * (_loss3 ** 2) + self.cf3, -1)
...
def build_generator(app_details_csv, models_dir, gui_information_dir, control_elements_id_dir, categories_app_emb, cutted_ui_elements, cutted_resized_ui_elements):
    ...
    x_ids2 = [[x[0], x[4], cutted_ui_elements + '\\' + os.path.basename(os.path.dirname(x[4])) + '\\' +
               os.path.basename(x[4]).split('_')[0] + '\\' +
